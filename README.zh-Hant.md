<p align="center">
  <img src="ironclaw.png?v=2" alt="IronClaw" width="200"/>
</p>

<h1 align="center">ironclaw-zh</h1>

<p align="center">
  <strong>IronClaw 中文本地化、本地優先、安全自託管發行版</strong>
</p>

<p align="center">
  <strong>Chinese-localized, local-first IronClaw distribution for secure self-hosting</strong>
</p>

<p align="center">
  <a href="#上游專案"><img src="https://img.shields.io/badge/upstream-nearai%2Fironclaw-181717.svg" alt="Upstream: nearai/ironclaw" /></a>
  <a href="#授權"><img src="https://img.shields.io/badge/license-MIT%20OR%20Apache%202.0-blue.svg" alt="License: MIT OR Apache-2.0" /></a>
</p>

<p align="center">
  <strong>語言切換：</strong>
  <a href="README.en.md">English</a> |
  <a href="README.md">简体中文</a> |
  <a href="README.zh-Hant.md">繁體中文</a>
</p>

---

`ironclaw-zh` 是基於 [nearai/ironclaw](https://github.com/nearai/ironclaw) 的社群維護發行版，重點面向中文體驗、本地優先部署與對上游安全架構的保留。

> 本專案為社群維護版本，不是 NEAR AI 官方發行版。

## 簡介

- 基於 IronClaw 的中文發行版。
- 面向本地優先和自託管使用情境。
- 提供中文文件、Web UI 本地化、預設與技能支援。

## 文件入口

- [專案概覽](docs/zh-Hans/overview.md)
- [安裝](docs/zh-Hans/install.md)
- [設定](docs/zh-Hans/configuration.md)
- [部署](docs/zh-Hans/deployment.md)
- [安全說明](docs/zh-Hans/security.md)
- [技能說明](docs/zh-Hans/skills.md)
- [常見問題](docs/zh-Hans/faq.md)
- [English README](README.en.md)
- [簡體中文 README](README.md)

## 版本

- 目前套件版本：`0.16.1`
- 目前建議發行標籤：`v0.16.1-zh.1`

## 上游專案

上游專案： [nearai/ironclaw](https://github.com/nearai/ironclaw)

本倉庫基於上游 IronClaw 維護，通用修復會盡量優先回提上游。

## 授權

本倉庫沿用上游授權模式：

- MIT
- Apache-2.0

詳見 [LICENSE-MIT](LICENSE-MIT) 與 [LICENSE-APACHE](LICENSE-APACHE)。
